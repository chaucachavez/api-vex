...
class PDF extends baseFpdf 
{
    ...
    {   
        ...
        $this->setXY(26, -28); 
        $this->Cell(180, 5, utf8_decode(''), $this->borde, 0, 'L');
        $this->Ln();
        $this->setX(26);
        $this->Cell(180, 5, utf8_decode('Representación impresa de la '. $this->comprobante.' ELECTRÓNICA, para ver el documento visita:'), $this->borde, 0, 'L');
        $this->Ln();
        $this->setX(26);
        $this->SetFont('Arial', '', 8.5); 
        $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
        $this->Cell(180, 5, utf8_decode('https://www.profactura.pe/comprobantes/' . $this->ruc), $this->borde, 0, 'L');
        $this->Ln();
        $this->setX(26);
        $this->SetFont('Arial', '', 8.5); 
        $this->SetTextColor(0, 0, 0);
        $this->Cell(28, 5, utf8_decode('Emitido mediante un'), $this->borde, 0, 'L');
        $this->SetFont('Arial', 'B', 8.5); 
        $this->Cell(52, 5, utf8_decode('Proveedor Autorizado por la SUNAT'), $this->borde, 0, 'L');
        $this->SetFont('Arial', '', 8.5); 
        $this->Cell(55, 5, utf8_decode('mediante Resolución de Intendencia No.'), $this->borde, 0, 'L');
        $this->SetFont('Arial', 'B', 8.5); 
        $this->Cell(45, 5, utf8_decode('034-005-0005315'), $this->borde, 0, 'L');
...
class invoiceA4Controller extends Controller 
{        
    ...
    {       
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetTextColor($pdf->color[0], $pdf->color[1], $pdf->color[2]);  
        if ($venta->iddocumentofiscal === 10 || $venta->iddocumentofiscal === 13) {
            $documentonc = '';
        ...
        $pdf->RoundedRect($pdf->getX(), $pdf->getY(), 202, 7, 1.5, '1234', 'DF');
        $pdf->Cell(100, 8, utf8_decode('DESCRIPCIÓN'), $pdf->borde, 0, 'L');
        $pdf->Cell(17, 8, utf8_decode('CÓD.'), $pdf->borde, 0, 'C'); 
        $pdf->Cell(17, 8, utf8_decode('CANT.'), $pdf->borde, 0, 'C');
        $pdf->Cell(17, 8, utf8_decode('UM'), $pdf->borde, 0, 'C');
        $pdf->Cell(17, 8, utf8_decode('V/U'), $pdf->borde, 0, 'C');
        $pdf->Cell(17, 8, utf8_decode('P/U'), $pdf->borde, 0, 'C');         
        $pdf->Cell(17, 8, utf8_decode('IMPORTE'), $pdf->borde, 0, 'C');
        $pdf->Ln(); 
        ...
        foreach ($venta->ventadet as $row) { 
            ...
            if ($pdf->getY() > 254) { // 254 = 297(HEIGHT) - 28(AUTOPAGEBREAK) - 15(MULTICELL)
                $pdf->AddPage();  
            }

            $y1 = $pdf->getY();
            $pdf->MultiCell(100, $height, utf8_decode($descripcion), 'B', 'L', true, 3); // - 15(MULTICELL)
            $y2temp = $pdf->getY();
            $hcelda = $y2temp - $y1;
            $pdf->setXY(104, $y1);
        ...
        if ($pdf->getY() > 249) { // 249 = 297(HEIGHT) - 28(AUTOPAGEBREAK) - 20(CELL de TOTALES)
            $pdf->AddPage();  
        }
        ...
        if ( ($pdf->getY() + $height) > 269) { // 249 = 297(HEIGHT) - 28(AUTOPAGEBREAK)
            $pdf->AddPage();  
        }
        ...
        { 
            if ($pdf->getY() > 260) { // 260 = 297(HEIGHT) - 28(AUTOPAGEBREAK) - 9(Inf.Adicional + OBS)
                $pdf->AddPage();
            }
