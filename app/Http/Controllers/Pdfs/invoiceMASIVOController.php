...
{        
    ...
    public function reporte($id, $return = false)
    { 
        ...
        $pathPdfMasivo = public_path() . DIRECTORY_SEPARATOR . 'empresa' . 
                        DIRECTORY_SEPARATOR . auth()->user()->idempresa . 
                        DIRECTORY_SEPARATOR . 'pdf-masivo' . DIRECTORY_SEPARATOR;

        $pdf = new FPDF_Merge();   // [A4:210 x 297] [A5:148 x 210] [TICKET: No existe medida standar]

        $data = Venta::select('idventa', 'pdf')
                ->where('idmasivo', $id)
                ->whereNotNull('pdf') 
                ->get();
 
        foreach ($data as $value) {
            $nombre = explode('/', $value->pdf);

            $fileName = $nombre[count($nombre) - 1];

            $nombreFile = $pathPdf . $fileName;
        ...
        if ($return) {
            ...
            $nombreFile = $id . '.pdf';
            $pdf->output($pathPdfMasivo . $nombreFile);    
            
            if (file_exists($pathPdfMasivo . $nombreFile)) 
            {
                //Importante retornar el nombre del archivo
